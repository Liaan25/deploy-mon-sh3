...
## Модель KAE и учётные записи мониторинга

Во всех местах, где это требуется, используется модель **KAE**, вычисляемая из `NAMESPACE_CI` вида `CIxxxx_CIyyyy`:

- `KAE=$(echo "$NAMESPACE_CI" | cut -d'_' -f2)` → например, `CI84324523`.
- На основе `KAE` формируются имена учётных записей мониторинга (создаются через RLM/IDM, не скриптом):
  - `${KAE}-lnx-mon_sys` – сервисная УЗ (nologin), под которой работают user‑юниты мониторинга.
  - `${KAE}-lnx-mon_ci` – CI/ТУЗ, из‑под которой запускается пайплайн/ручные операции.

Скрипт `deploy_monitoring_script.sh`:

- **не создаёт** эти УЗ, а только **ожидает** их наличия;
- в начале работы через RLM‑сценарий `UVS_LINUX_ADD_USERS_GROUP` гарантирует, что обе УЗ состоят в группе `as-admin` (см. `ensure_monitoring_users_in_as_admin` в коде и раздел про RLM в `SECURITY_IB_NOTES.md`).
