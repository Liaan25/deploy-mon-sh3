...
# Прямое применение исправления к функции create_service_account_via_api
# Исправляем основную проблему: неправильное извлечение HTTP кода из ответа curl

echo "=== ПРЯМОЕ ПРИМЕНЕНИЕ ИСПРАВЛЕНИЯ ==="
echo

# Создаем исправленную версию функции
FIXED_FUNCTION=$(cat << 'EOF'
        ...
        create_service_account_via_api() {
            ...
            # ИСПРАВЛЕНИЕ: Используем тот же формат что и в тестовом скрипте
...
echo "Исправленная функция готова к применению."
echo
echo "=== КЛЮЧЕВЫЕ ИСПРАВЛЕНИЯ ==="
echo "1. ✅ Формат curl: Используем 'HTTP_CODE:%{http_code}' вместо просто '%{http_code}'"
echo "2. ✅ Извлечение кода: grep 'HTTP_CODE:' | cut -d: -f2 вместо tail -1"
echo "3. ✅ Извлечение тела: grep -v 'HTTP_CODE:' вместо head -n -1"
echo "4. ✅ Упрощена логика: Удалены лишние проверки и вложенные функции"
echo
echo "=== КАК ПРИМЕНИТЬ ==="
echo "1. Откройте файл deploy_monitoring_script.sh в редакторе"
echo "2. Найдите строку: 'create_service_account_via_api() {'"
echo "3. Замените ВСЮ функцию до закрывающей '}' на:"
echo
echo "$FIXED_FUNCTION"
echo
echo "=== РЕЗУЛЬТАТ ==="
echo "После исправления функция будет работать так же как тестовый скрипт"
